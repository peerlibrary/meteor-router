  ...
  */
  Meteor.Router.Route.prototype.pathWithContext = function (context) {
    ...
    var replacePathPartWithContextValue = function (part, i) {
      var re = new RegExp(part.replace(/[?]/g, "\\?"), "g"),
          prop = part.replace(/[:?]/g, ""),
          val;
    ...
    // Remove duplicate // which can happen because of optional parts
    path = path.replace(/\/\/+/g, "/");
    // Remove trailing / which can happen because of optional parts,
    // but only if this is not the whole path
    if (path !== "/")
      path = path.replace(/\/$/, "");
